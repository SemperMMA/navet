...
import type { MusicItem, MusicSourceAdapter } from '@navet/core/music';
import { Music2 } from 'lucide-react';

export function getMusicSourceAccent(source: MusicSourceAdapter | undefined) {
  const color = source?.presentation?.accentColor;
  return color && /^#[0-9a-f]{6}$/i.test(color) ? color : 'currentColor';
}

export function MusicSourceIcon({
  source,
  className,
}: {
  ...
  const icon = source.presentation?.icon ?? source.id;
...
export function formatMusicDuration(durationMs?: number) {
  if (!durationMs || durationMs < 0) return '';
  const totalSeconds = Math.floor(durationMs / 1000);
  return `${Math.floor(totalSeconds / 60)}:${String(totalSeconds % 60).padStart(2, '0')}`;
}

export function MusicArtwork({ item, className }: { item: MusicItem; className: string }) {
